#!/usr/bin/env python3
"""synthetic_sharp_turn_path.py -- [테스트용] 직진하다가 좁은 반경으로
급하게 꺾고 다시 직진하는 합성 경로. 실차에서 급조향 시 관성/슬립 때문에
로봇이 코너를 밖으로 밀리며 도는지(understeer), 아니면 MPPI가 아예 곡선을
못 따라가고 직진으로 관통해버리는지("곡선 관통" 회귀 버그) 보는 용도.

synthetic_track_loop_path.py/synthetic_circle_path.py와 달리 폐루프가
아니다 -- 접근 구간(approach_m) -> 급회전(turn_deg, turn_radius_m) ->
이탈 구간(exit_m) 순서의 1회성 경로. 로봇이 경로 끝에 다다르면 window가
자연히 짧아지며 거기서 멈춘다(다시 안 돎).

turn_radius_m을 작게 줄수록(예: 0.3~0.5m) "급하게" 꺾인다 -- 실제 로봇이
그 반경으로 물리적으로 돌 수 있는지와는 별개로, MPPI에게 그렇게 요구하는
참조 경로를 준다는 뜻.
...
사용 예:
    python3 synthetic_sharp_turn_path.py
    python3 synthetic_sharp_turn_path.py --ros-args -p topic:=/path -p turn_deg:=90.0 -p turn_radius_m:=0.4
    python3 synthetic_sharp_turn_path.py --ros-args -p turn_deg:=-90.0   # 우회전
"""
import math
...
def generate_arc(x0, y0, yaw0, radius_m, direction, sweep_deg, num_points):
...
def generate_sharp_turn_path(
        x0, y0, yaw0, approach_m, turn_deg, turn_radius_m, exit_m,
        points_per_m):
    pts = [(x0, y0)]
    ...
    turn_points = max(8, int(abs(math.radians(turn_deg)) * turn_radius_m * points_per_m))
    seg = generate_arc(x1, y1, yaw_after_approach, turn_radius_m, direction,
                        abs(turn_deg), turn_points)
    pts.extend(seg)
...
def nearest_point_index(points, x, y, min_idx=0):
    ...
    되돌아갈 일이 없으므로, 전체 경로에서 찾으면 급회전처럼 경로가 자기
    자신과 가까워지는 구간에서 위치 오차/슬립 때문에 반대쪽 구간이 유클리드
    거리상 더 가깝게 잡혀 window가 이미 지나온 지점으로 되돌아가고, 그 결과
    같은 구간을 무한 반복하는 문제가 생긴다(synthetic_half_circle_path.py와
    동일 버그, 동일 수정)."""
...
class SyntheticSharpTurnPublisher(Node):
    def __init__(self):
        super().__init__('synthetic_sharp_turn_publisher')

        self.declare_parameter('approach_m', 2.0)
        self.declare_parameter('turn_deg', 90.0)  # 양수=좌회전, 음수=우회전
        ...
        if self.topic == '/path':
            self.get_logger().warn(
                "topic='/path'로 설정됨: 실제 로봇이 이 급회전 경로를 그대로 "
        ...
        self.get_logger().info(
            f"synthetic_sharp_turn_publisher 시작: topic='{self.topic}' "
            f"approach={self.approach_m}m turn={self.turn_deg}deg@r={self.turn_radius_m}m "
            f"exit={self.exit_m}m -- {self.frame_id}->{self.base_frame} TF 필요."
        )
    ...
    def _publish_path(self):
        ...
        if self._path_points is None:
            self._path_points = generate_sharp_turn_path(
                x, y, yaw, self.approach_m, self.turn_deg, self.turn_radius_m,
...
def main(args=None):
    ...
    node = SyntheticSharpTurnPublisher()
    try:
        rclpy.spin(node)
