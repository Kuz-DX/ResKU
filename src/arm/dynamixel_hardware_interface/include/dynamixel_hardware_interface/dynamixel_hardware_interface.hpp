...
private:
  ...
  std::map<std::string, std::pair<double, double>> position_limits_;
  struct JointPositionCalibration
  {
    double zero_offset{0.0};
    double direction{1.0};
    bool state_wraparound{false};
    bool command_wraparound{false};
    bool command_nearest_turn{false};
  };
  std::map<std::string, JointPositionCalibration> joint_position_calibrations_;
  std::map<std::string, double> last_model_positions_;
