  ...
  bool is_fast)
{
  ...
  // The 100 Hz controller period is only 10 ms, while a Linux USB-serial
  // adapter may buffer data for up to 16 ms by default.  Keep the control
  // period unchanged, but allow enough time to receive every status packet in
  // a group read.  Apply the minimum during activation as well, where
  // period_ms is zero.
  constexpr double kMinGroupReadTimeoutMs = 30.0;
  port_handler->setPacketTimeout(
    period_ms > kMinGroupReadTimeoutMs ? period_ms : kMinGroupReadTimeoutMs);
