...
public:
  ...
  /**
    * @brief perception -> MPPI path pipeline 진단용 (controller.hpp의
    * debug_path_pipeline_ 참고). true면 transformPath()가 매 cycle
    * closest-point distance / prune 후 plan 크기를 RCLCPP_INFO_THROTTLE로
    * 로그한다. transformPath() 자체의 동작(반환값)은 이 플래그와 무관하게
    * 전혀 바뀌지 않는다 -- /transformed_global_plan(기존 visualize_ 토픽,
    * trajectory_visualizer.cpp 참고)이 이미 이 함수의 반환값을 그대로
    * publish하므로 그 데이터 자체를 위해 별도 topic을 새로 만들지는 않는다.
    */
  void setDebugPathPipeline(bool debug) {debug_path_pipeline_ = debug;}
