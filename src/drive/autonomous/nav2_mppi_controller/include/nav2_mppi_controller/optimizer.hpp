...
protected:
  ...
  // doldrive_ws fix (2026-08-08): reset() used to be invoked directly from
  // ParametersHandler's dynamic-parameter callback, which runs on the ROS
  // executor thread -- a different thread than the FollowPath action's
  // control-loop worker thread that calls optimize()/updateControlSequence().
  // A parameter change (even an incidental one, e.g. controller_server's own
  // "verbose" param being set right around activation) racing against an
  // in-flight optimize() call resizes state_/control_sequence_/costs_ out
  // from under it -- reproducibly a SIGSEGV. Fix: the callback only raises
  // this flag; the control-loop thread consumes it and calls reset() itself,
  // at the start of optimize(), so reset() always runs on the same thread
  // that uses these buffers. Same pattern as critics_data_.fail_flag/fallback().
