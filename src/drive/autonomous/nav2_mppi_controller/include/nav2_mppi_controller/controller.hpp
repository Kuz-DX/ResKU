...
protected:
  ...
  // perception -> MPPI path pipeline 진단용 (doldrive_ws, path pipeline
  // instrumentation). true일 때만 setPlan()에 들어온 path를
  // /debug/mppi_setplan_path로 republish하고, 호출 빈도/간격을
  // RCLCPP_INFO_THROTTLE로 로그. critic/optimizer/PathHandler 로직 자체는
  // 이 플래그와 무관하게 전혀 바뀌지 않는다.
  bool debug_path_pipeline_{false};
  std::shared_ptr<rclcpp_lifecycle::LifecyclePublisher<nav_msgs::msg::Path>> setplan_debug_pub_;
  size_t setplan_call_count_{0};
  rclcpp::Time last_setplan_time_;
};
