...
void Optimizer::getParams()
{
  ...
  // doldrive_ws fix (2026-08-08): don't call reset() directly from here --
  // this runs on the ROS executor thread, a different thread than the one
  // that calls optimize()/updateControlSequence() (see optimizer.hpp
  // needs_reset_ comment for the full race explanation). Just raise the
  // flag; optimize() picks it up on the control-loop thread.
  parameters_handler_->addPostCallback(
    [this]() {needs_reset_.store(true, std::memory_order_release);});
...
{
  // doldrive_ws fix (2026-08-08): consume the deferred reset request here,
  // on the control-loop thread, instead of letting the parameter callback
  // call reset() directly from the executor thread. See needs_reset_ in
  // optimizer.hpp for the race this avoids.
