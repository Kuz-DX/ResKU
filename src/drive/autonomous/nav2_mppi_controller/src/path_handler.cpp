  ...
  const geometry_msgs::msg::PoseStamped & robot_pose)
{
  ...
  if (debug_path_pipeline_) {
    // step 4 통계: /transformed_global_plan(trajectory_visualizer.cpp가 이
    // 반환값 그대로 publish)이 이미 실제 path 데이터를 보여주므로, 여기서는
    // 그 topic만으로 안 보이는 값(가장 가까운 경로점까지 거리, prune 전/후
    // 크기)만 로그한다. lower_bound는 아래 prunePlan()으로 invalidate되므로
    // 그 전에 거리 계산.
    using nav2_util::geometry_utils::euclidean_distance;
    const bool has_closest = lower_bound != global_plan_up_to_inversion_.poses.end();
    const double closest_dist = has_closest ? euclidean_distance(global_pose, *lower_bound) : -1.0;
    RCLCPP_INFO_THROTTLE(
      logger_, *clock_, 500,
      "[debug_path_pipeline] PathHandler: global_plan_size=%zu closest_dist=%.3f "
      "transformed_plan_size=%zu",
      global_plan_up_to_inversion_.poses.size(), closest_dist, transformed_plan.poses.size());
  }
