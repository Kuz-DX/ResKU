...
// slope_traverse_spring_node: slope_traverse_pland_node.cpp(PlanD)를 그대로
// 복사한 봄 미션(mission_spring_drive.launch.py) 전용 노드.
//
// [2026-09-05 신규, 사용자 요청] PlanD의 IDLE/SLOPE_DRIVE/SLOPE_EXIT/RECOVERY
// 로직은 전혀 안 건드리고, 그 앞에 STARTUP 상태 하나만 추가했다 -- 노드
// 시작 즉시(side/path/IMU 등 아무 조건도 안 보고) initial_straight_speed_mps_
// (기본 0.4m/s)로 initial_straight_drive_sec_(기본 3초, CLI에서
// mission_spring_drive.launch.py의 initial_straight_drive_sec 인자로 조정
// 가능) 동안 무조건 직진한 뒤 IDLE로 넘어가서 그 다음부터는 PlanD와
// 완전히 동일하게(side 신호 대기 -> SLOPE_DRIVE -> ...) 동작한다. 배경:
// 대회 트랙 출발선에서 인지팀 신호/경로가 아직 안정화되기 전에 초반
// 몇 초는 그냥 밀고 나가야 하는 구간이 있어서(사용자 확인).
//
...
// SlopeTraverseSpringNode, node name "slope_traverse_planc_node" ->
...
// 만료되며 MPPI 제어로 복귀한다.
class SlopeTraverseSpringNode : public rclcpp::Node
{
public:
  SlopeTraverseSpringNode()
  : Node("slope_traverse_spring_node")
  {
    track_width_m_ = declare_parameter<double>("track_width_m", 0.4904);

    // --- [봄 미션 전용] STARTUP: 노드 시작 직후 무조건 직진 -----------------
    // mission_spring_drive.launch.py에서 initial_straight_drive_sec 런치
    // 인자로 CLI에서 바로 조정 가능(예: initial_straight_drive_sec:=5.0).
    ...
    imu_sub_ = create_subscription<sensor_msgs::msg::Imu>(
      "/imu", 10, std::bind(&SlopeTraverseSpringNode::imuCallback, this, std::placeholders::_1));
    ...
    camera_imu_sub_ = create_subscription<sensor_msgs::msg::Imu>(
      ...
      std::bind(&SlopeTraverseSpringNode::cameraImuCallback, this, std::placeholders::_1));
    slope_side_sub_ = create_subscription<std_msgs::msg::Int8>(
      slope_side_topic_, 10,
      std::bind(&SlopeTraverseSpringNode::slopeSideCallback, this, std::placeholders::_1));
    path_sub_ = create_subscription<nav_msgs::msg::Path>(
      path_topic_, rclcpp::QoS(1),
      std::bind(&SlopeTraverseSpringNode::pathCallback, this, std::placeholders::_1));
    ...
    control_timer_ = create_wall_timer(
      ...
      std::bind(&SlopeTraverseSpringNode::controlTimerCallback, this));
    ...
    param_callback_handle_ = add_on_set_parameters_callback(
      std::bind(&SlopeTraverseSpringNode::onParamUpdate, this, std::placeholders::_1));
  ...
  void updateState()
  {
    ...
    switch (state_) {
      case State::STARTUP: {
        ...
        // 다 되면 곧바로 IDLE로 -- 그 이후부터는 PlanD와 완전히 동일하게
        // side 신호를 기다린다.
...
int main(int argc, char ** argv)
{
  ...
  rclcpp::spin(std::make_shared<SlopeTraverseSpringNode>());
  rclcpp::shutdown();
  return 0;
}
