...
// slope_traverse_planc_spring_node: slope_traverse_planc_node.cpp(PlanC)를
// 그대로 복사한 뒤, mission_spring_drive.launch.py가 PlanD(slope_traverse_
// pland_node) 위에 STARTUP 상태를 얹어 slope_traverse_spring_node를 만든 것과
// 동일한 방식으로, PlanC에도 같은 STARTUP만 추가한 봄 구간 전용 사본이다.
//
// [2026-09-05 신규, 사용자 요청] 아래 원본(PlanC) 로직은 전혀 안 건드리고,
// 그 앞에 STARTUP 상태 하나만 추가했다 -- 노드 시작 즉시(side/path/IMU 등
// 아무 조건도 안 보고) initial_straight_speed_mps_(기본 0.4m/s)로
// initial_straight_drive_sec_(기본 3초, CLI에서 mission_spring_autonomous_
// drive.launch.py의 initial_straight_drive_sec 인자로 조정 가능) 동안 무조건
// 직진한 뒤 IDLE로 넘어가서, 그 이후부터는 PlanC와 완전히 동일하게 동작한다
// (side 신호 대기 -> SLOPE_DRIVE -> SLOPE_EXIT -> RECOVERY). 배경: 대회 트랙
// 출발선에서 인지팀 신호/경로가 아직 안정화되기 전에 초반 몇 초는 그냥 밀고
// 나가야 하는 구간이 있어서(slope_traverse_spring_node.cpp와 동일 근거).
//
// [주의] updateState()는 has_imu_data_==false면 맨 위에서 조기 return하므로
// (원본 그대로) STARTUP -> IDLE 전환도 IMU 데이터가 들어온 뒤에만 이뤄진다.
// 다만 controlTimerCallback()의 STARTUP 분기 자체는 updateState() 결과와
// 무관하게 state_==STARTUP이기만 하면 무조건 직진 명령을 낸다 -- IMU가 아직
// 하나도 안 들어와도(state_ 초기값이 STARTUP이므로) 이 직진은 그대로 나간다
// (slope_traverse_spring_node.cpp와 동일 동작, 의도적으로 손대지 않음).
//
// ---- 아래는 slope_traverse_planc_node.cpp(PlanC) 원본 docstring 그대로 ----
//
...
// 만료되며 MPPI 제어로 복귀한다.
class SlopeTraversePlanCSpringNode : public rclcpp::Node
{
public:
  SlopeTraversePlanCSpringNode()
  : Node("slope_traverse_planc_spring_node")
  {
    ...
    // mission_spring_autonomous_drive.launch.py에서 initial_straight_drive_sec
    // 런치 인자로 CLI에서 바로 조정 가능(예: initial_straight_drive_sec:=5.0).
    initial_straight_drive_sec_ =
      declare_parameter<double>("initial_straight_drive_sec", 3.0);
    initial_straight_speed_mps_ =
      declare_parameter<double>("initial_straight_speed_mps", 0.4);
    ...
    imu_sub_ = create_subscription<sensor_msgs::msg::Imu>(
      "/imu", 10, std::bind(&SlopeTraversePlanCSpringNode::imuCallback, this, std::placeholders::_1));
    ...
    camera_imu_sub_ = create_subscription<sensor_msgs::msg::Imu>(
      ...
      std::bind(&SlopeTraversePlanCSpringNode::cameraImuCallback, this, std::placeholders::_1));
    slope_side_sub_ = create_subscription<std_msgs::msg::Int8>(
      slope_side_topic_, 10,
      std::bind(&SlopeTraversePlanCSpringNode::slopeSideCallback, this, std::placeholders::_1));
    path_sub_ = create_subscription<nav_msgs::msg::Path>(
      path_topic_, rclcpp::QoS(1),
      std::bind(&SlopeTraversePlanCSpringNode::pathCallback, this, std::placeholders::_1));
    ...
    control_timer_ = create_wall_timer(
      ...
      std::bind(&SlopeTraversePlanCSpringNode::controlTimerCallback, this));
    ...
    tunable_params_ = {
      ...
      {"initial_straight_drive_sec", &initial_straight_drive_sec_},
      {"initial_straight_speed_mps", &initial_straight_speed_mps_},
    };
    param_callback_handle_ = add_on_set_parameters_callback(
      std::bind(&SlopeTraversePlanCSpringNode::onParamUpdate, this, std::placeholders::_1));

    publishState();

    RCLCPP_INFO(
      get_logger(),
      "slope_traverse_planc_spring_node started: initial_straight_drive_sec=%.1fs @ "
      "%.2fm/s, then slope_side_topic=%s climb_speed=%.2f "
      "kp_steer=%.2f wz_steer_max=%.2f climb_straight_grace_sec=%.1f",
      initial_straight_drive_sec_, initial_straight_speed_mps_,
      slope_side_topic_.c_str(), climb_speed_, kp_steer_, wz_steer_max_,
      climb_straight_grace_sec_);
  }

private:
  enum class State { STARTUP, IDLE, SLOPE_DRIVE, SLOPE_EXIT, RECOVERY };
  ...
  void updateState()
  {
    ...
    switch (state_) {
      case State::STARTUP: {
        // [봄 미션 전용] 노드 시작 즉시, side/path 등 아무 조건도 안 보고
        // initial_straight_drive_sec_(기본 3초, CLI로 조정 가능) 동안
        // 무조건 직진(controlTimerCallback()의 STARTUP 분기 참고). 시간이
        // 다 되면 곧바로 IDLE로 -- 그 이후부터는 PlanC와 완전히 동일하게
        // side 신호를 기다린다.
        if ((now - state_entered_time_).seconds() >= initial_straight_drive_sec_) {
          state_ = State::IDLE;
        }
        break;
      }
  ...
  void controlTimerCallback()
  {
    ...
    if (state_ == State::STARTUP) {
      // [봄 미션 전용] side/path/IMU 등 아무 것도 안 보고 무조건 직진.
      state_enum = 4;
      target_for_debug = 0.0;
      cmd.linear.x = initial_straight_speed_mps_;
      cmd.angular.z = 0.0;
    } else if (state_ == State::SLOPE_DRIVE) {
      state_enum = 1;
  ...
  // [봄 미션 전용] STARTUP
  double initial_straight_drive_sec_;
  double initial_straight_speed_mps_;
...
int main(int argc, char ** argv)
{
  ...
  rclcpp::spin(std::make_shared<SlopeTraversePlanCSpringNode>());
  rclcpp::shutdown();
  return 0;
}
