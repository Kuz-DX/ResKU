...
// slope_traverse_plane_node: slope_traverse_pland_node.cpp(PlanD)를 그대로
// 복사한 "PlanE" 변형.
//
// [2026-09-05 신규, 사용자 요청] PlanD 코드/파라미터를 전부 그대로 가져오고
// 딱 하나만 바꿨다 -- SLOPE_EXIT의 wz 상한 역산에 쓰는 차동조향 dps 쌍
// (slope_exit_side_motor_dps_/slope_exit_other_motor_dps_)을 400/150에서
// 300/150으로. `computeExitWzMax()`(wzFromDpsPair 참고) 기준 wz 상한이
// ≈1.0rad/s에서 ≈0.75rad/s로 줄어든다(더 보수적인 조향). PlanD의 나머지
// 로직(1회성 pitch 트리거 래치, side==0 폴백 제거, 시간 기반 정지-래치
// 제거 등)은 전부 그대로다 -- 상세 배경은 아래 PlanD 이력 참고.
//
...
//
// (이상은 PlanD 시절 이력 -- PlanE는 위에서 밝힌 400/150 -> 300/150 dps
// 변경 하나만 추가로 얹었다.) 클래스/노드 이름은 SlopeTraversePlanDNode ->
// SlopeTraversePlanENode, node name "slope_traverse_pland_node" ->
// "slope_traverse_plane_node"로 바꿨다.
//
...
// 만료되며 MPPI 제어로 복귀한다.
class SlopeTraversePlanENode : public rclcpp::Node
{
public:
  SlopeTraversePlanENode()
  : Node("slope_traverse_plane_node")
  {
    ...
    slope_exit_end_pitch_deg_ =
      declare_parameter<double>("slope_exit_end_pitch_deg", -3.0);
    ...
    // [PlanE] 400.0 -> 300.0 (사용자 요청) -- computeExitWzMax() 상한이
    // ≈1.0rad/s에서 ≈0.75rad/s로 줄어듦.
    slope_exit_side_motor_dps_ = declare_parameter<double>("slope_exit_side_motor_dps", 300.0);
    ...
    imu_sub_ = create_subscription<sensor_msgs::msg::Imu>(
      "/imu", 10, std::bind(&SlopeTraversePlanENode::imuCallback, this, std::placeholders::_1));
    ...
    camera_imu_sub_ = create_subscription<sensor_msgs::msg::Imu>(
      ...
      std::bind(&SlopeTraversePlanENode::cameraImuCallback, this, std::placeholders::_1));
    slope_side_sub_ = create_subscription<std_msgs::msg::Int8>(
      slope_side_topic_, 10,
      std::bind(&SlopeTraversePlanENode::slopeSideCallback, this, std::placeholders::_1));
    path_sub_ = create_subscription<nav_msgs::msg::Path>(
      path_topic_, rclcpp::QoS(1),
      std::bind(&SlopeTraversePlanENode::pathCallback, this, std::placeholders::_1));
    ...
    control_timer_ = create_wall_timer(
      ...
      std::bind(&SlopeTraversePlanENode::controlTimerCallback, this));
    ...
    param_callback_handle_ = add_on_set_parameters_callback(
      std::bind(&SlopeTraversePlanENode::onParamUpdate, this, std::placeholders::_1));
...
int main(int argc, char ** argv)
{
  ...
  rclcpp::spin(std::make_shared<SlopeTraversePlanENode>());
  rclcpp::shutdown();
  return 0;
}
