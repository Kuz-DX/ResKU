#!/usr/bin/env python3
"""
autonomous.planz_spring.py

[2026-09-05 신규] autonomous.planz.py(PlanZ, 2026-08-27 시점 purepursuit.py
복원본)를 그대로 복사한 뒤, mission_spring_drive.launch.py의
slope_traverse_spring_node(PlanD + STARTUP)와 동일한 취지로 STARTUP 상태
하나만 얹은 봄 구간 전용 사본. autonomous.planz.py 원본은 "그 시점 코드를
그대로 되살린 독립 사본"이라는 존재 이유상 손대지 않고, 이 파일만 따로
갈라져 나왔다 -- 원본과 서로 수정을 공유하지 않는다.

추가된 동작(STARTUP): 노드 시작 즉시 /path 등 아무 조건도 안 보고
initial_straight_drive_sec(기본 3초) 동안 무조건 직진(initial_straight_speed_mps,
기본 0.4m/s)한 뒤, 그 이후부터는 원본 autonomous.planz.py와 완전히 동일하게
/path 기반 pure pursuit으로 넘어간다. 배경(mission_spring_drive.launch.py와
동일): 대회 트랙 출발선에서 인지팀 신호/경로가 아직 안정화되기 전에 초반
...
파라미터 (실측/현장 튜닝 전 placeholder 다수):
    ...
    [2026-09-05 신규, 이 사본 전용] initial_straight_drive_sec/
    initial_straight_speed_mps -- 모듈 상단 설명 참고.

실행:
    ros2 run robot_bringup autonomous.planz_spring.py
...
class PurePursuitNode(Node):
    def __init__(self):
        ...
        # [2026-09-05 신규, 이 사본 전용] mission_spring_drive.launch.py의
        # slope_traverse_spring_node STARTUP 상태와 동일한 취지 -- 모듈
        # 상단 설명 참고.
        self.declare_parameter('initial_straight_drive_sec', 3.0)
        self.declare_parameter('initial_straight_speed_mps', 0.4)
        ...
        self.initial_straight_drive_sec = max(
            0.0, float(p('initial_straight_drive_sec').value))
        self.initial_straight_speed_mps = float(p('initial_straight_speed_mps').value)
        ...
        # [2026-09-05 신규] STARTUP 종료 시각 기준점 -- 노드 생성 시각부터
        # initial_straight_drive_sec 동안은 /path 상태와 무관하게 무조건 직진.
        self._node_start_time = self.get_clock().now()
        self._startup_done_logged = False
        ...
        self.get_logger().info(
            f'purepursuit_node (spring/STARTUP) started: {path_topic} -> {self.base_frame}, '
            f'initial_straight_drive_sec={self.initial_straight_drive_sec}s @ '
            f'{self.initial_straight_speed_mps}m/s, then lookahead={self.lookahead_distance_m}m, '
            f'v_target={self.target_linear_speed_m_s}m/s, '
    ...
    def _control_loop(self):
        # [2026-09-05 신규] STARTUP -- 노드 시작 후 initial_straight_drive_sec
        # 동안은 /path 등 아무 조건도 안 보고 무조건 직진(mission_spring_drive
        # .launch.py의 slope_traverse_spring_node와 동일한 취지). 이 구간이
        # 끝나면 아래 원본 autonomous.planz.py 로직으로 그대로 넘어간다.
        elapsed_s = (self.get_clock().now() - self._node_start_time).nanoseconds * 1e-9
        if elapsed_s < self.initial_straight_drive_sec:
            left_dps, right_dps = self._skid_steer_to_dps(
                self.initial_straight_speed_mps, 0.0)
            msg = Float32MultiArray()
            msg.data = [left_dps, right_dps]
            self.cmd_pub.publish(msg)
            return
        if not self._startup_done_logged:
            self._startup_done_logged = True
            self.get_logger().info(
                f'STARTUP straight drive done ({self.initial_straight_drive_sec}s) -- '
                'switching to /path pure pursuit.')
